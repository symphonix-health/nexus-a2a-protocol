"""In-process unit tests for the pharmacy agent.

Loads the pharmacy agent's FastAPI app directly via importlib and routes
requests through httpx ASGITransport — no live services required.

The pharmacy agent uses build_generic_demo_app which handles
pharmacy/recommend and pharmacy/check_interactions via the declared
methods mechanism. No OpenAI calls are made by this agent in unit mode.
"""
...
# ── Matrix helpers ────────────────────────────────────────────────────────────
_MATRIX_FILE = _PROJECT_ROOT / "HelixCare" / "pharmacy_agent_unit_matrix.json"
...
# ── Agent app loader ──────────────────────────────────────────────────────────
_AGENT_MAIN = (
    _PROJECT_ROOT / "demos" / "helixcare" / "pharmacy-agent" / "app" / "main.py"
)


def _load_agent_module() -> ModuleType:
    """Load pharmacy agent FastAPI app from its file path via importlib."""
    os.environ.setdefault("NEXUS_JWT_SECRET", _SECRET)
    os.environ.setdefault("NEXUS_AGENT_LLM_ENABLED", "false")

    spec = importlib.util.spec_from_file_location("pharmacy_agent_main", str(_AGENT_MAIN))
    mod = importlib.util.module_from_spec(spec)
    sys.modules["pharmacy_agent_main"] = mod
...
async def test_pharmacy_positive(scenario: dict, client: AsyncClient, valid_headers: dict):
...
async def test_pharmacy_negative(scenario: dict, client: AsyncClient, valid_headers: dict):
    payload = scenario["input_payload"]
...
async def test_pharmacy_edge(scenario: dict, client: AsyncClient, valid_headers: dict):
    payload = scenario["input_payload"]
    headers = _headers_for_scenario(scenario, valid_headers)
    status, body = await _rpc(client, headers, payload)
...
async def test_pharmacy_agent_card(client: AsyncClient):
...
async def test_pharmacy_health_endpoint(client: AsyncClient):
...
@pytest.mark.parametrize(
    "method, condition, allergies",
    [
        ("pharmacy/recommend", "hypertension", []),
        ("pharmacy/recommend", "type 2 diabetes", ["penicillin"]),
        ("pharmacy/recommend", "community-acquired pneumonia", ["penicillin", "sulfa"]),
        ("pharmacy/check_interactions", "monitoring", []),
        ("pharmacy/check_interactions", "monitoring", []),
    ],
    ids=[
        "recommend-hypertension",
        "recommend-diabetes-allergy",
        "recommend-pneumonia-multi-allergy",
        "check-interactions-warfarin-aspirin",
        "check-interactions-empty-list",
    ],
)
async def test_pharmacy_domain_methods(
    method: str,
    condition: str,
    allergies: list,
    client: AsyncClient,
    ...
    """Directly probe pharmacy domain methods with various condition/allergy combos."""
    if method == "pharmacy/recommend":
        params = {
            "task": {
                "patient": {"patient_id": "P-PHARM-DOMAIN"},
                "condition": condition,
                "allergies": allergies,
            }
        }
    ...
    payload = {
        "jsonrpc": "2.0",
        "id": f"domain-pharm-{method.split('/')[-1]}",
        "method": method,
        "params": params,
    }
    status, body = await _rpc(client, valid_headers, payload)
    assert status == 200, (
        f"method={method}, condition={condition}: expected 200, got {status}; body={body}"
    )
