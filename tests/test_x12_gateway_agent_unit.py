"""In-process unit tests for the x12-gateway agent.

Loads the x12-gateway agent's FastAPI app directly via importlib and
routes requests through httpx ASGITransport — no live services required.

The x12-gateway agent is an interop stub built on build_generic_demo_app.
...
# ── Matrix helpers ────────────────────────────────────────────────────────────
_MATRIX_FILE = _PROJECT_ROOT / "HelixCare" / "x12_gateway_agent_unit_matrix.json"
...
def _load_agent_module() -> ModuleType:
    """Load x12-gateway agent FastAPI app from its file path via importlib."""
    os.environ.setdefault("NEXUS_JWT_SECRET", _SECRET)
    os.environ.setdefault("NEXUS_AGENT_LLM_ENABLED", "false")

    spec = importlib.util.spec_from_file_location(
        "x12_gateway_agent_main", str(_AGENT_MAIN)
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["x12_gateway_agent_main"] = mod
...
async def client(agent_app):
    async with AsyncClient(
        ...
        base_url="http://x12-gateway-agent",
    ) as c:
        yield c
...
async def test_x12_gateway_positive(
...
async def test_x12_gateway_negative(
    scenario: dict, client: AsyncClient, valid_headers: dict
):
    payload = scenario["input_payload"]
...
async def test_x12_gateway_edge(
    scenario: dict, client: AsyncClient, valid_headers: dict
):
    payload = scenario["input_payload"]
    headers = _headers_for_scenario(scenario, valid_headers)
...
async def test_x12_gateway_agent_card(client: AsyncClient):
...
async def test_x12_gateway_health_endpoint(client: AsyncClient):
