"""In-process unit tests for the care-coordinator agent.

Loads the care-coordinator's FastAPI app directly via importlib and routes
requests through httpx ASGITransport — no live services required.

The care-coordinator is built with build_generic_demo_app() and handles only
the standard tasks/* protocol methods (no custom domain method is declared).
...
# ── Matrix helpers ────────────────────────────────────────────────────────────
_MATRIX_FILE = _PROJECT_ROOT / "HelixCare" / "care_coordinator_unit_matrix.json"
...
# ── Agent app loader ──────────────────────────────────────────────────────────
_AGENT_MAIN = (
    _PROJECT_ROOT / "demos" / "helixcare" / "care-coordinator" / "app" / "main.py"
)


def _load_agent_module() -> ModuleType:
    """Load care-coordinator FastAPI app from its file path via importlib."""
    os.environ.setdefault("NEXUS_JWT_SECRET", _SECRET)
    os.environ.setdefault("NEXUS_AGENT_LLM_ENABLED", "false")

    spec = importlib.util.spec_from_file_location(
        "care_coordinator_main", str(_AGENT_MAIN)
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["care_coordinator_main"] = mod
...
async def client(agent_app):
    async with AsyncClient(
        ...
        base_url="http://care-coordinator",
    ) as c:
        yield c
...
async def test_care_coordinator_positive(
...
async def test_care_coordinator_negative(
    scenario: dict, client: AsyncClient, valid_headers: dict
):
    payload = scenario["input_payload"]
...
async def test_care_coordinator_edge(
    scenario: dict, client: AsyncClient, valid_headers: dict
):
    payload = scenario["input_payload"]
    headers = _headers_for_scenario(scenario, valid_headers)
...
async def test_care_coordinator_health_endpoint(client: AsyncClient):
...
async def test_care_coordinator_agent_card(client: AsyncClient):
    ...
    # care-coordinator only declares standard tasks/* methods
    assert "tasks/sendSubscribe" in methods, (
        "Agent card must declare tasks/sendSubscribe"
    )
