"""In-process unit tests for the fhir-profile agent.

Loads the fhir-profile agent's FastAPI app directly via importlib and
routes requests through httpx ASGITransport — no live services required.

The fhir-profile agent is an interop stub built on build_generic_demo_app.
...
# ── Matrix helpers ────────────────────────────────────────────────────────────
_MATRIX_FILE = _PROJECT_ROOT / "HelixCare" / "fhir_profile_agent_unit_matrix.json"
...
def _load_agent_module() -> ModuleType:
    """Load fhir-profile agent FastAPI app from its file path via importlib."""
    os.environ.setdefault("NEXUS_JWT_SECRET", _SECRET)
    os.environ.setdefault("NEXUS_AGENT_LLM_ENABLED", "false")

    spec = importlib.util.spec_from_file_location(
        "fhir_profile_agent_main", str(_AGENT_MAIN)
    )
    mod = importlib.util.module_from_spec(spec)
    sys.modules["fhir_profile_agent_main"] = mod
...
async def client(agent_app):
    async with AsyncClient(
        ...
        base_url="http://fhir-profile-agent",
    ) as c:
        yield c
...
async def test_fhir_profile_positive(
...
async def test_fhir_profile_negative(
    scenario: dict, client: AsyncClient, valid_headers: dict
):
    payload = scenario["input_payload"]
...
async def test_fhir_profile_edge(
    scenario: dict, client: AsyncClient, valid_headers: dict
):
    payload = scenario["input_payload"]
    headers = _headers_for_scenario(scenario, valid_headers)
...
async def test_fhir_profile_agent_card(client: AsyncClient):
...
async def test_fhir_profile_health_endpoint(client: AsyncClient):
