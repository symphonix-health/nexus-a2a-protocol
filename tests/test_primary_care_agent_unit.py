"""In-process unit tests for the primary-care agent.

Loads the primary-care agent's FastAPI app directly via importlib and routes
...
tasks/* protocol methods and the domain method primary_care/manage_visit.
...
# ── Matrix helpers ────────────────────────────────────────────────────────────
_MATRIX_FILE = _PROJECT_ROOT / "HelixCare" / "primary_care_agent_unit_matrix.json"
...
# ── Agent app loader ──────────────────────────────────────────────────────────
_AGENT_MAIN = (
    _PROJECT_ROOT / "demos" / "helixcare" / "primary-care-agent" / "app" / "main.py"
)


def _load_agent_module() -> ModuleType:
    """Load primary-care agent FastAPI app from its file path via importlib."""
    os.environ.setdefault("NEXUS_JWT_SECRET", _SECRET)
    # Prevent any downstream RPC calls from hanging
    os.environ.setdefault("NEXUS_AGENT_LLM_ENABLED", "false")

    spec = importlib.util.spec_from_file_location("primary_care_agent_main", str(_AGENT_MAIN))
    mod = importlib.util.module_from_spec(spec)
    sys.modules["primary_care_agent_main"] = mod
...
async def client(agent_app):
    async with AsyncClient(
        ...
        base_url="http://primary-care-agent",
    ) as c:
        yield c
...
async def test_primary_care_positive(scenario: dict, client: AsyncClient, valid_headers: dict):
    payload = scenario["input_payload"]
    # Skip GET-based scenarios in the RPC parametrized sweep
...
async def test_primary_care_negative(scenario: dict, client: AsyncClient, valid_headers: dict):
    payload = scenario["input_payload"]
...
async def test_primary_care_edge(scenario: dict, client: AsyncClient, valid_headers: dict):
    payload = scenario["input_payload"]
    headers = _headers_for_scenario(scenario, valid_headers)
    status, body = await _rpc(client, headers, payload)
...
async def test_primary_care_health_endpoint(client: AsyncClient):
...
async def test_primary_care_agent_card(client: AsyncClient):
    ...
    assert "primary_care/manage_visit" in methods, (
        "Agent card must declare primary_care/manage_visit"
    )
