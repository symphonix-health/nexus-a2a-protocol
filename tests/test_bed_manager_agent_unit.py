"""In-process unit tests for the bed-manager agent.

Loads the bed-manager agent's FastAPI app directly via importlib and routes
requests through httpx ASGITransport — no live services required.

The bed-manager agent uses build_generic_demo_app which handles
admission/assign_bed and admission/check_availability via the declared
...
# ── Matrix helpers ────────────────────────────────────────────────────────────
_MATRIX_FILE = _PROJECT_ROOT / "HelixCare" / "bed_manager_agent_unit_matrix.json"
...
# ── Agent app loader ──────────────────────────────────────────────────────────
_AGENT_MAIN = (
    _PROJECT_ROOT / "demos" / "helixcare" / "bed-manager-agent" / "app" / "main.py"
)


def _load_agent_module() -> ModuleType:
    """Load bed-manager agent FastAPI app from its file path via importlib."""
    os.environ.setdefault("NEXUS_JWT_SECRET", _SECRET)
    os.environ.setdefault("NEXUS_AGENT_LLM_ENABLED", "false")

    spec = importlib.util.spec_from_file_location("bed_manager_agent_main", str(_AGENT_MAIN))
    mod = importlib.util.module_from_spec(spec)
    sys.modules["bed_manager_agent_main"] = mod
...
async def client(agent_app):
    async with AsyncClient(
        ...
        base_url="http://bed-manager-agent",
    ) as c:
        yield c
...
async def test_bed_manager_positive(scenario: dict, client: AsyncClient, valid_headers: dict):
...
async def test_bed_manager_negative(scenario: dict, client: AsyncClient, valid_headers: dict):
    payload = scenario["input_payload"]
...
async def test_bed_manager_edge(scenario: dict, client: AsyncClient, valid_headers: dict):
    payload = scenario["input_payload"]
    headers = _headers_for_scenario(scenario, valid_headers)
    status, body = await _rpc(client, headers, payload)
...
async def test_bed_manager_agent_card(client: AsyncClient):
...
async def test_bed_manager_health_endpoint(client: AsyncClient):
...
@pytest.mark.parametrize(
    "method, ward, priority",
    [
        ("admission/assign_bed", "cardiology", "urgent"),
        ("admission/assign_bed", "orthopaedics", "routine"),
        ("admission/assign_bed", "icu", "critical"),
        ("admission/check_availability", "general", None),
        ("admission/check_availability", "maternity", None),
    ],
    ids=[
        "assign-cardiology-urgent",
        "assign-orthopaedics-routine",
        "assign-icu-critical",
        "check-general",
        "check-maternity",
    ],
)
async def test_bed_manager_domain_methods(
    method: str,
    ward: str,
    priority: str | None,
    client: AsyncClient,
    ...
    """Directly probe bed-manager domain methods with various ward/priority combos."""
    params: dict = {"task": {"patient": {"patient_id": "P-BED-DOMAIN"}, "ward": ward}}
    if priority:
        params["task"]["priority"] = priority

    payload = {
        "jsonrpc": "2.0",
        "id": f"domain-{ward}-{priority}",
        "method": method,
        "params": params,
    }
    status, body = await _rpc(client, valid_headers, payload)
    assert status == 200, f"method={method}, ward={ward}: expected 200, got {status}; body={body}"
