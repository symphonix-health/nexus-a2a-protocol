"""In-process unit tests for the CCM (Chronic Care Management) agent.

Loads the ccm-agent's FastAPI app directly via importlib and routes
...
tasks/* protocol methods and the domain method ccm/monthly_review.
...
# ── Matrix helpers ────────────────────────────────────────────────────────────
_MATRIX_FILE = _PROJECT_ROOT / "HelixCare" / "ccm_agent_unit_matrix.json"
...
# ── Agent app loader ──────────────────────────────────────────────────────────
_AGENT_MAIN = (
    _PROJECT_ROOT / "demos" / "helixcare" / "ccm-agent" / "app" / "main.py"
)


def _load_agent_module() -> ModuleType:
    """Load ccm-agent FastAPI app from its file path via importlib."""
    os.environ.setdefault("NEXUS_JWT_SECRET", _SECRET)
    os.environ.setdefault("NEXUS_AGENT_LLM_ENABLED", "false")

    spec = importlib.util.spec_from_file_location("ccm_agent_main", str(_AGENT_MAIN))
    mod = importlib.util.module_from_spec(spec)
    sys.modules["ccm_agent_main"] = mod
...
async def test_ccm_negative(scenario: dict, client: AsyncClient, valid_headers: dict):
    payload = scenario["input_payload"]
...
async def test_ccm_edge(scenario: dict, client: AsyncClient, valid_headers: dict):
    payload = scenario["input_payload"]
    headers = _headers_for_scenario(scenario, valid_headers)
    status, body = await _rpc(client, headers, payload)
...
async def test_ccm_health_endpoint(client: AsyncClient):
...
async def test_ccm_agent_card(client: AsyncClient):
    ...
    assert "ccm/monthly_review" in methods, (
        "Agent card must declare ccm/monthly_review"
    )
