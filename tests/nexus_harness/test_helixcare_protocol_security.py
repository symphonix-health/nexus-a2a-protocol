"""Matrix-driven HelixCare tests -- helixcare_protocol_security_matrix.json
...
MATRIX = "helixcare_protocol_security_matrix.json"
...
@pytest.mark.asyncio
async def test_helixcare_security_positive(scenario: dict, client: httpx.AsyncClient, auth_headers: dict):
    sr = ScenarioResult(
...
@pytest.mark.asyncio
async def test_helixcare_security_negative(scenario: dict, client: httpx.AsyncClient, auth_headers: dict):
    sr = ScenarioResult(
        use_case_id=scenario["use_case_id"],
        scenario_title=scenario["scenario_title"],
        poc_demo=scenario["poc_demo"],
    ...
    try:
        ...
        if payload.get("protocol_step") in ("mqtt_subscribe", "agent_card_get", "did_verify"):
            sr.status = "skip"
