...
def main():
    ...
    log("=" * 60)
    packages_to_remove = [
        "fastapi", "pydantic", "pydantic-core", "starlette", 
        "uvicorn", "httptools", "anyio", "httpx",
    ...
    log("=" * 60)
    rc = run(f'"{VENV_PYTHON}" -c "from fastapi import FastAPI; import pydantic; print(f\'FastAPI+Pydantic {{pydantic.__version__}} OK\')"')
    if rc != 0:
        log("FATAL: FastAPI import still broken!")
        return 1
    
    rc = run(f'"{VENV_PYTHON}" -c "from starlette.testclient import TestClient; print(\'TestClient OK\')"')
